...
namespace UniMerge {
	public static class Util {
		...
		public delegate void FoldoutDraw();
		...
		public static bool PropSame;
		//Looks like Unity already has one of these... doesn't work the way I want though
		public static IEnumerable PropEqual(SerializedProperty mine, SerializedProperty theirs, GameObject mineParent, GameObject theirsParent) {
				...
				if(mine.propertyType == theirs.propertyType) {
					switch(mine.propertyType) {
						case SerializedPropertyType.AnimationCurve:
							if(mine.animationCurveValue.keys.Length == theirs.animationCurveValue.keys.Length) {
								for(int i = 0; i < mine.animationCurveValue.keys.Length; i++) {
									if(mine.animationCurveValue.keys[i].inTangent != theirs.animationCurveValue.keys[i].inTangent) {PropSame = false;yield break;}
									if(mine.animationCurveValue.keys[i].outTangent != theirs.animationCurveValue.keys[i].outTangent)
										{PropSame = false;yield break;}
							...
							PropSame = true;yield break;
						case SerializedPropertyType.ArraySize:
							//Debug.LogWarning("Got ArraySize type");
							break;
						case SerializedPropertyType.Boolean:
							PropSame = mine.boolValue == theirs.boolValue;yield break;
#if !(UNITY_3_0 || UNITY_3_0_0 || UNITY_3_1 || UNITY_3_2 || UNITY_3_3 || UNITY_3_4 || UNITY_3_5)
						case SerializedPropertyType.Bounds:
							PropSame =  mine.boundsValue == theirs.boundsValue;yield break;
#endif
						...
						case SerializedPropertyType.Color:
							PropSame =  mine.colorValue == theirs.colorValue;yield break;
						case SerializedPropertyType.Enum:
							PropSame =  mine.enumValueIndex == theirs.enumValueIndex;yield break;
						case SerializedPropertyType.Float:
							PropSame =  mine.floatValue == theirs.floatValue;yield break;
							...
							if(mine.isArray) {
								if(mine.arraySize == theirs.arraySize) {
									for(int i = 0; i < mine.arraySize; i++) {
										foreach(IEnumerable e in PropEqual(mine.GetArrayElementAtIndex(i),
											theirs.GetArrayElementAtIndex(i), mineParent, theirsParent)){
												yield return e;
											if(!PropSame)
												yield break;
										}
									...
									{PropSame = true;yield break;}
								} else {PropSame = false;yield break;}
							} else{
								foreach(IEnumerable e in CheckChildren(mine, theirs, mineParent, theirsParent))
									yield return e;
							}
#endif
							break;
						...
						case SerializedPropertyType.Integer:
							PropSame =  mine.intValue == theirs.intValue;yield break;
							...
							if(mine.objectReferenceValue && theirs.objectReferenceValue) {
								...
								if(t == typeof(GameObject)) {
									...
									if(ChildOfParent((GameObject)mine.objectReferenceValue, mineParent)
										...
										PropSame = mine.objectReferenceValue.name == theirs.objectReferenceValue.name;yield break;
									}
								}
							 ...
							 PropSame = mine.objectReferenceValue == theirs.objectReferenceValue;yield break;
						case SerializedPropertyType.Rect:
							 PropSame = mine.rectValue == theirs.rectValue;yield break;
						case SerializedPropertyType.String:
							 PropSame = mine.stringValue == theirs.stringValue;yield break;
						case SerializedPropertyType.Vector2:
							 PropSame = mine.vector2Value == theirs.vector2Value;yield break;
						case SerializedPropertyType.Vector3:
							 PropSame =  mine.vector3Value == theirs.vector3Value;yield break;
#if !(UNITY_3_0 || UNITY_3_0_0 || UNITY_3_1 || UNITY_3_2 || UNITY_3_3 || UNITY_3_4 || UNITY_3_5)
						case (SerializedPropertyType)16:
							PropSame =  mine.quaternionValue.Equals(theirs.quaternionValue);yield break;
#endif
						default:
		...
		private static IEnumerable CheckChildren(SerializedProperty mine, SerializedProperty theirs, GameObject mineParent, GameObject theirsParent) {
			bool enter = true;
			if(mine == null && theirs == null)
				yield break;
			if(mine == null || theirs == null) {
			...
			while(mTmp.Next(enter)) {
				...
				if(mTmp.depth != mine.depth)	//Once we're back in the same depth, we've gone too far
					break;
				foreach(IEnumerable e in PropEqual(mTmp, tTmp, mineParent, theirsParent))
					yield return e;
				if(!PropSame)
					yield break;
			}
